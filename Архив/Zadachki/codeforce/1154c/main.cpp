...
#define CALENDAR {fishFood, rabbitStew, chickenChops, fishFood, \
chickenChops, rabbitStew, fishFood}
...
void createFoodCalendar(kitty *foodCalendar) {
  foodCalendar[0] = initializingStructKitty(0, 0, 0);
  catEatingToday calendar[] = CALENDAR;
  for (int i = 0; i < DAYS_IN_WEEK; i++) {
    foodCalendar[i + 1] = foodCalendar[i];
    if (calendar[i] == fishFood)
      foodCalendar[i + 1].fish += 1;
    else if (calendar[i] == rabbitStew)
  ...
  for (int i = 0; i <= DAYS_IN_WEEK; i++) {
    for (int j = 0; j <= DAYS_IN_WEEK; j++) {
      if (i < j) {
        ...
        if (isEnoughFood(foodRemained, needFood))
          maxOverflowDays = max(maxOverflowDays,
                                (long long) (j + DAYS_IN_WEEK - i));
      }
    }
  }
